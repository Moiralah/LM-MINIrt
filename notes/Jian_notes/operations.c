#include "stdio.h"
#include "stdlib.h"
#include "math.h"

double	magnitude(double *vector, int size)
{
	double	mag;
	int	i;

	mag = 0.0;
	i = -1;
	while (++i < size)
		mag += (vector[i] * vector[i]);
	return (sqrt(mag));
}

double	*cross(double *vector1, double *vector2, int size)
{
	double	*newVector;

	newVector = calloc(size, sizeof(double));
	newVector[0] = (vector1[1] * vector2[2]) - (vector2[1] * vector1[2]);
	newVector[1] = (vector2[0] * vector1[2]) - (vector1[0] * vector2[2]);
	newVector[2] = (vector1[0] * vector2[1]) - (vector2[0] * vector1[1]);
	return (newVector);
}

double	*norm(double *vector, int size)
{
	double	*newVector;
	double	mag;
	int	i;

	i = -1;
	mag = magnitude(vector, size);
	newVector = calloc(size, sizeof(double));
	while (++i < size)
		newVector[i] = vector[i] / mag;
	return (newVector);
}

double	*mult(double *vector, double value, int size)
{
	double	*newVector;
	int	i;

	i = -1;
	newVector = calloc(size, sizeof(double));
	while (++i < size)
		newVector[i] = vector[i] * value;
	return (newVector);
}

double	*sub(double *vector1, double *vector2, int size)
{
	double	*newVector;
	int	i;

	i = -1;
	newVector = calloc(size, sizeof(double));
	while (++i < size)
		newVector[i] = vector1[i] - vector2[i];
	return (newVector);
}

double	*add(double *vector1, double *vector2, int size)
{
	double	*newVector;
	int	i;

	i = -1;
	newVector = calloc(size, sizeof(double));
	while (++i < size)
		newVector[i] = vector1[i] + vector2[i];
	return (newVector);
}

int	dot(double *vector1, double *vector2, int size)
{
	int	product;
	int	i;

	product = 0;
	i = -1;
	while (++i < size)
		product += (vector1[i] * vector2[i]);
	return (product);
}

void	printVector(double *vector, int size)
{
	int	i;

	i = -1;
	printf("[");
	while (++i < (size - 1))
		printf("%f ", vector[i]);
	printf("%f]\n", vector[i]);
	free(vector);
}

int	main(void)
{
	double	vector1[3];
	double	vector2[3];

	vector1[0] = 1;
	vector1[1] = 2;
	vector1[2] = 3;
	vector2[0] = 4;
	vector2[1] = 5;
	vector2[2] = 6;
	printVector(add(vector1, vector2, 3), 3);
	printVector(sub(vector1, vector2, 3), 3);
	printVector(sub(vector2, vector1, 3), 3);
	printVector(mult(vector1, 2, 3), 3);
	printVector(mult(vector1, 0.5, 3), 3);
	printf("%f\n", magnitude(vector1, 3));
	printVector(norm(vector1, 3), 3);
	printf("%f\n", magnitude(norm(vector1, 3), 3));
	printf("%d\n", dot(vector1, vector2, 3));
	printVector(cross(vector1, vector2, 3), 3);
	printf("%f\n", magnitude(cross(vector1, vector2, 3), 3));
	return (0);
}
